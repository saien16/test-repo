...
const SPRITES = (() => {
  ...
  // コントラスト高め＝「基地の部屋」がはっきり見えるように。
  function hubBg() {
    const rack = (x, w) => `<rect x="${x}" y="6" width="${w}" height="40" rx="2" fill="#15323e" stroke="#2f6a7a" stroke-width="0.8"/>` +
      `<g fill="#3fd08a">${[11, 17, 23, 29, 35, 41].map(y => `<rect x="${x + 2}" y="${y}" width="${w - 4}" height="1.8" rx="0.9" opacity="${(Math.floor(x) + y) % 3 ? 0.95 : 0.45}"/>`).join('')}</g>` +
      `<circle cx="${x + w - 3}" cy="9" r="1.2" fill="#7CFFB2"/><circle cx="${x + 3}" cy="9" r="1.2" fill="#ffd34a"/>`;
    const floor = [4, 22, 40, 60, 78, 96].map(x => `<line x1="${x}" y1="60" x2="${(x - 50) * 2.1 + 50}" y2="100"/>`).join('') +
      [64, 70, 78, 88].map(y => `<line x1="0" y1="${y}" x2="100" y2="${y}"/>`).join('');
    return `<svg class="hub-bg-svg" viewBox="0 0 100 100" preserveAspectRatio="none">` +
      // 奥の壁パネル
      `<rect x="0" y="0" width="100" height="52" fill="#0e2230" opacity="0.6"/>` +
      `<g opacity="0.9">${rack(4, 16)}${rack(23, 16)}${rack(61, 16)}${rack(80, 16)}</g>` +
      // ネオン管（グロー付き）
      `<line x1="0" y1="52" x2="100" y2="52" stroke="#22f0d0" stroke-width="2.4" opacity="0.25"/>` +
      `<line x1="0" y1="52" x2="100" y2="52" stroke="#5cffe8" stroke-width="0.9" opacity="0.85"/>` +
      // 床グリッド（パース）
      `<g stroke="#1f8a5e" stroke-width="0.6" opacity="0.8">${floor}</g></svg>`;
  }
