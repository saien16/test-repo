...
function listActions(g) {
  ...
  can('ci_recon', '🛠️ 防御の穴あけ（3T 敵が柔らかく）', g.reconDone && g.res.info >= 20, g.reconDone ? '📡20 不足' : '先に偵察',
      '敵の堅さ↓で撃破が通りやすい 📡20 🚨+5');
  can('ci_diag', '🧪 弱点解析（3T 攻撃が刺さる）', g.res.tech >= 15, '🧬15 不足',
      '命中・会心↑で撃破が刺さる 🧬15 🚨+5');
...
if (typeof document !== 'undefined' && document.getElementById) {
  ...
  function defensePanel(g) {
    ...
    return '<div class="defrow">' +
      '<span class="def-title">🛡️ 硬さ<small>低いほど刺さる</small></span>' +
  ...
  function renderBattle() {
    ...
    const cutBadges =
      (g.cut.hHack > 0 ? '<span class="badge ci">🛠️敵-柔化(' + g.cut.hHack + 'T)</span>' : '') +
      (g.cut.vDiag > 0 ? '<span class="badge ci">🧪攻-鋭化(' + g.cut.vDiag + 'T)</span>' : '');

    // 行動をカテゴリ分けしてタブ化（コンテンツが増えてもボタンの壁にしない）
    const renderBtn = (a) => {
      const kind = a.id.startsWith('card:') ? ' card' : (a.id.startsWith('strike:') ? ' strike' : '')
        + (/^(penetrate|break):/.test(a.id) ? ' danger' : ''); // 発覚度が大きく跳ねる強行手は警告色
      let spr = '';
    ...
    // 初回プレイ＋各ステージ初挑戦は「王道順」の次の一手を光らせる（迷子防止）
    // 順: 偵察→侵入→(弱点発見済みなら)エクスプロイト→スキャン→横展開→撃破
    let recId = null;
    if (!onboarded || g._firstTry) {
    ...
    $('battle-root').innerHTML =
      '<div class="bt-top">' +
        '<div class="hud">' +
          ...
          '<span class="pill info" title="情報: 偵察・スキャンに使う">📡 情報 ' + g.res.info + '</span>' +
          '<span class="pill tech" title="技術: 高度な技・カードに使う">🧬 技術 ' + g.res.tech + '</span>' +
          '<span class="pill resr" title="資源: 過負荷・強行に使う">💾 資源 ' + g.res.res + '</span>' +
        '</div>' +
        bar('🚨 発覚度(100で駆除)', g.warning, 100, 'warn', sh.warn) + warnNote +
      '</div>' +
      '<div class="bt-main">' +
        '<div class="bt-left">' +
          ...
          '<div class="boss">' +
            ...
            (function () {
              const goal = Math.round(d.initTotal * g.stage.win.ratio);      // ここまで削ればクリア
              const cur = d.C + d.I + d.A;
              const remainNow = Math.max(0, Math.round(cur - goal));         // あと削る量
              const remainSh = Math.max(0, Math.round(shTotal - goal));
              const prog = Math.min(100, Math.round((1 - Math.max(0, cur - goal) / Math.max(1, d.initTotal - goal)) * 100));
              return '<div class="goal-chip">🎯 このサーバを乗っ取れ！ 守りを <b class="ci-total" data-from="' + remainSh +
                '" data-to="' + remainNow + '">' + remainSh + '</b> 削ればクリア' +
                '<span class="goal-prog">達成 ' + prog + '%</span></div>';
            })() +
  ...
  function grantWin() {
    ...
    if (lockedC.length && _rng() < 0.30) {
      ...
      unlockedCards.push(nc.id); saveJSON('malcore.cards', unlockedCards); markNew(nc.id); loot.card = nc.name;
    }
    // 低確率ドロップ: マルウェア（12%）
    ...
    if (lockedM.length && _rng() < 0.12) {
      ...
      unlockedMal.push(nm.id); saveJSON('malcore.malunlocked', unlockedMal); refreshRoster(); markNew(nm.id); loot.malware = nm.name;
    }
    G._loot = loot;
    return loot;
  }
  ...
  function renderResult() {
    ...
    if (!onboarded && win) { onboarded = true; saveJSON('malcore.onboarded', true); } // 初"勝利"で手引きを卒業（負けた初心者には助けを残す）
    let reward = '';
    if (win) {
  ...
  let triedStages = loadJSON('malcore.tried', {}); // 各ステージ初挑戦か（初挑戦は段数/新ゲート導入で迷うのでおすすめ手を再点灯）
  ...
  // ===== コインマイナー＆採掘（攻略後にステージへ仕込まれ、数回の出撃の間だけ微量採掘） =====
  let miners = loadJSON('malcore.miners', {}); // stageId → 残り出撃回数
  let stock = Object.assign({ btc: 0, info: 0, tech: 0, res: 0 }, loadJSON('malcore.stock', {}));  // 回収済み（改良/開発・次出撃に使用）
  let pending = Object.assign({ btc: 0, info: 0, tech: 0, res: 0 }, loadJSON('malcore.pending', {})); // 未回収の採掘物
  let newItems = loadJSON('malcore.new', []); // 図鑑の未読NEW（ドロップ/開発で追加、図鑑を開くとクリア）
  function saveStock() { saveJSON('malcore.stock', stock); }
  function savePending() { saveJSON('malcore.pending', pending); }
  function pendingTotal() { return pending.btc + pending.info + pending.tech + pending.res; }
  ...
  // 出撃のたびに全マイナーが採掘→"未回収"へ。残り0で駆除。母港で能動回収する。
  function tickMiners() {
    const rep = { btc: 0, sites: 0, cleaned: [] };
    Object.keys(miners).forEach(sid => {
      if (miners[sid] <= 0) { delete miners[sid]; return; }
      const y = minerYield(sid);
      pending.btc += y.btc; pending.info += y.info; pending.tech += y.tech; pending.res += y.res;
      rep.btc += y.btc; rep.sites++;
      miners[sid]--;
      if (miners[sid] <= 0) { delete miners[sid]; rep.cleaned.push(sid); }
    });
    saveJSON('malcore.miners', miners); savePending();
    return rep;
  }
  // 母港で採掘物を回収（未回収→ストックへ）
  function collectMining() {
    stock.btc += pending.btc; stock.info += pending.info; stock.tech += pending.tech; stock.res += pending.res;
    const got = { btc: pending.btc, info: pending.info, tech: pending.tech, res: pending.res };
    pending = { btc: 0, info: 0, tech: 0, res: 0 };
    saveStock(); savePending();
    return got;
  }
  ...
  function markNew(id) { if (newItems.indexOf(id) < 0) { newItems.push(id); saveJSON('malcore.new', newItems); } }
  function clearNew() { if (newItems.length) { newItems = []; saveJSON('malcore.new', newItems); } }
  ...
  function renderStageDetail(i) {
    ...
    el.innerHTML =
      ...
      '<button id="sd-sortie" class="big-btn' + (onboarded ? '' : ' rec') + '">🦠 このステージへ出撃' +
        (onboarded ? '' : '<span class="rec-badge">👉おすすめ</span>') + '</button>';
    const btn = $('sd-sortie');
  ...
  function startGame() {
    const rep = tickMiners(); // 出撃のたびに既設マイナーが"未回収"へ採掘（母港で回収）
    G = createGame(STAGES[selectedStage], equipped, levels, party, portBuffs);
    const sid = STAGES[selectedStage].id;
    G._firstTry = !triedStages[sid];                 // このステージ初挑戦なら手引きを出す
    if (G._firstTry) { triedStages[sid] = true; saveJSON('malcore.tried', triedStages); }
    ...
    if (!onboarded) {
      G.log.push('🔰 光る「👉おすすめ」を順に押そう');
      G.log.push('🔰 📡情報＝偵察・スキャン ／ 🧬技術＝技・カード ／ 💾資源＝強行。🚨が100で駆除＝負け');
    } else if (G._firstTry) {
      G.log.push('🔰 新ステージ初挑戦。光る「👉おすすめ」が次の一手だよ');
    }
  ...
  function renderBriefing() {
    ...
    const body = briefTab === 'equip'
      ...
      : '<h2 class="sub">編成（出撃 ' + party.length + '/3）<small>₿ ' + stock.btc + '（改良に使用）</small></h2>' +
        handHtml;
    const rookieTip = onboarded ? '' : '<p class="rookie-tip">🔰 最初はこのままでOK。編成は2回目から。下の光る「出撃」を押そう</p>';
    $('briefing-body').innerHTML =
      '<p class="intro">' + stage.intro + '</p>' + rookieTip + tabs + body;
    // 初回は出撃ボタンを光らせ、母港→ステージ→戦闘まで「光るボタンを押すだけ」を貫通させる
    const sortieBtn = $('btn-sortie');
    if (sortieBtn) { sortieBtn.classList.toggle('rec', !onboarded); }
  ...
  function renderHome() {
    ...
    const newN = newItems.length;
    const buffCosts = PORT_BUFFS.map(b => (portBuffs[b.key] || 0) < b.max ? b.cost[portBuffs[b.key] || 0] : Infinity);
    const minBuff = Math.min.apply(null, buffCosts);
    const lockedMalCosts = MAL.filter(m => !malUnlocked(m.id)).map(m => devCost(m));
    const minDevMal = lockedMalCosts.length ? Math.min.apply(null, lockedMalCosts) : Infinity;
    const devReady = (devP >= minBuff) || (stock.btc >= minDevMal);   // 開発で"今できる"ことがある
    const collectBtn = pend > 0
      ? '<button id="home-collect" class="big-btn collect">⛏️ 採掘物を回収する（₿' + pending.btc +
        ' 📡' + pending.info + ' 🧬' + pending.tech + ' 💾' + pending.res + '）</button>'
      : '';
    // 今やるべき1手（迷子防止）: 回収 > 初勝利 > 強化 > 再設置 > 進行
    const nextAct = (function () {
      if (pend > 0) return '👉 まずは<b>採掘物を回収</b>しよう';
      if (!onboarded) return '👉 <b>出撃</b>して初勝利を目指そう';
      if (devReady) return '👉 <b>強化</b>できるよ（改良・開発・母港バフ）';
      if (mining === 0) return '👉 <b>出撃</b>してマイナーを再設置しよう';
      return '👉 <b>出撃</b>して次のステージへ';
    })();
    const dash =
      '<div class="home-next">' + nextAct + '</div>' +
      collectBtn +
      '<button id="home-sortie" class="big-btn">⚔️ 出撃する（敵サーバを乗っ取る）</button>' +
      '<div class="home-actions">' +
      '<button class="home-act" data-go="codex">📖 図鑑を見る' + (newN ? '<span class="act-badge">NEW ' + newN + '</span>' : '') +
      '<small>集めた攻撃と対策を見る</small></button>' +
      '<button class="home-act" data-go="develop">🛠️ 強化する' + (devReady && onboarded ? '<span class="act-badge dot">!</span>' : '') +
    ...
    $('home-body').innerHTML =
      ...
      '<div class="home-right">' +
        ...
        dash +
        '<div class="home-card">' +
        '<p class="home-meta cur">₿ <b>' + stock.btc + '</b>＝開発・改良 ／ 🛠️ <b>' + devP + '</b>＝母港バフ ／ 📡🧬💾＝次の出撃の弾（貯めるほど楽）</p>' +
        '<p class="home-meta">🦠 母港 ' + unlockedMal.length + '/' + MAL.length +
        ' ／ 🗺️ 解放ステージ ' + unlockedStages + '/' + STAGES.length + ' ／ 📖 図鑑 ' + unlockedCards.length + '/' + CARDS.length + '</p>' +
        (function () {
          const act = Object.keys(miners).filter(k => miners[k] > 0);
          if (!act.length) return '<p class="home-meta">⛏️ 採掘拠点: なし（出撃してクリアで設置）</p>';
          const list = act.map(k => (STAGES.find(s => s.id === k) || {}).name + ' <b>あと' + miners[k] + '出撃</b>').join(' ／ ');
          return '<p class="home-meta">⛏️ 採掘拠点（出撃で採掘が進む）: ' + list + '</p>';
        })() +
    ...
    const cb = $('home-collect');
    if (cb) cb.addEventListener('click', () => {
      Sound.unlock(); Sound.play('cutin');
      const got = collectMining();
      spawnLootBurst({ reward: { info: got.info, tech: got.tech, res: got.res }, btc: got.btc }); // 放置報酬も勝利時と同格の宝箱演出に
      showToast('⛏️ 回収！ ₿+' + got.btc + ' 📡+' + got.info + ' 🧬+' + got.tech + ' 💾+' + got.res);
      setTimeout(renderHome, 650); // 宝箱が開く演出を見せてから再描画
    });
    const hs = $('home-sortie');
    if (hs) hs.addEventListener('click', () => { Sound.unlock(); Sound.play('select'); renderStageSelect(); show('stage-screen'); });
    $('home-body').querySelectorAll('.home-act').forEach(b =>
      b.addEventListener('click', () => {
        Sound.unlock(); Sound.play('select');
        const go = b.dataset.go;
        if (go === 'codex') { clearNew(); renderCodex(); show('codex-screen'); }
        else if (go === 'develop') { renderDevelop(); show('develop-screen'); }
      }));
  ...
  function initUI() {
    ...
    tap('btn-codex', () => { Sound.play('select'); clearNew(); renderCodex(); show('codex-screen'); });
    ...
    const goTab = (id) => {
      ...
      else if (id === 'codex-screen') { clearNew(); renderCodex(); }
