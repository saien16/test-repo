...
const entryTech = (M.STAGE.nodes[M.STAGE.path[1]] || {}).entry;
assert(gf.flash && entryTech && gf.flash.name === entryTech, '初期侵害=ステージ毎の侵入口の技名(' + entryTech + ')');
...
const g0 = M.createGame(M.STAGE);
const labels = M.listActions(g0).map(a => a.label).join(' ');
const entryTech0 = (g0.stage.nodes[g0.stage.path[1]] || {}).entry;
assert(/ポートスキャニング/.test(labels) && entryTech0 && labels.indexOf(entryTech0) >= 0, '行動ボタンが技名表記になっている');
...
console.log('シナリオ23: コンテンツ倫理lint（抽象化契約）');
{
  ...
  MALS.forEach(m => scan('マルウェア「' + m.name + '」', m, ['name', 'desc', 'waza.name', 'waza.defense',
    'voice.attack', 'voice.crit', 'voice.hurt', 'voice.idle',
    'intel.L0', 'intel.notable', 'intel.real', 'intel.refs'])); // 声・図鑑intelも倫理lintの対象
  ...
  CARDSL.forEach(c => scan('カード「' + c.name + '」', c, ['name', 'desc', 'defense',
    'intel.L0', 'intel.notable', 'intel.real', 'intel.refs']));
