...
describe('パストラバーサルの封じ込め', () => {
  ...
  it('設定ファイル由来の baselinePath ではリポジトリ外を読まない', async () => {
    const pathSources: PathSources = { baselinePath: 'config-file', ignorePath: 'config-file' , reportDir: 'default', kevPath: 'default' };
    const { findings, errors } = await manageFindings(
      [makeRaw()],
      makeContext(),
  ...
  it('CLI由来（pathSources=cli）ならリポジトリ外のベースラインを読む', async () => {
    const pathSources: PathSources = { baselinePath: 'cli', ignorePath: 'default' , reportDir: 'default', kevPath: 'default' };
