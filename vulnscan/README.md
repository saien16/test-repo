...
> **はじめて使う方へ**: [はじめてのガイド](../docs/getting-started.md) から読んでください。
> 準備・実行・**レポートの読み方**・CIへの組み込みを順に説明しています。
> このREADMEは全オプションのリファレンスです。
...
この判定は `--fail-on` とは独立です。`--fail-on never` にしても効きます。
あれは検出結果でビルドを落とさない設定であって、壊れた走査を成功扱いにする設定ではないからです。
無効化するなら `--no-fail-on-incomplete`（または `failOnIncompleteScan: false`）を使ってください。
...
新旧が揃っていれば常に新しい方を使います。
旧名を使ったときは移行を促す警告が1行出るだけで、動作は変わりません。

## 走査の流れ

7つのステージを順に通し、共有モデルを育てていきます。
途中のステージが失敗しても、可能な限り部分結果を返します（何も出ないより途中まで出る方が役に立つため）。
...
| ⑤ | アーキテクチャ推定 | 構造把握 | マニフェストからの事実収集＋LLMによる構成・露出度の推測 |
| ⑥ | ヒートマップ生成 | 熱図描画 | 実測層（検出＝事実）×想定層（推測）を重ね、死角を洗い出す |
| ⑦ | レポート生成 | 編纂 | 優先度付け・要約・各形式へのレンダリング |

④と⑤にデータ依存は無いので並列に走ります。
⑥は⑤の結果が無ければ生成しません（推測の土台が無いのに想定リスクは語れないため）。
ステージ順の定義元は `src/core/orchestrator.ts` の `STAGE_SEQUENCE` 1箇所です。
...
| ステージ | 数えるもの | 理由 |
| --- | --- | --- |
| ① 索敵 | ファイル | 前処理がファイル数に比例する |
| ② 解析詠唱 | タスク → 候補 | 走査(レンズ×チャンク)と自己検証で**母数が変わる**ため局面名を併記 |
| ③ 照合 | 依存脆弱性 | 時間がかかるのは OSV への往復だけ |
| ④ 連鎖演算 | グループ | 1グループ = LLM 1回 |
| ⑤ 構造把握 | — | 時間の大半が LLM 1回の応答待ちで、内側に刻みが無い |
| ⑥ 熱図描画 | — | 純粋な同期関数で一瞬終わる |
| ⑦ 編纂 | — | ⑤と同じ |

⑤⑥⑦で進捗を出さないのは配線漏れではなく判断です。
即座に100%になったまま数十秒固まるバーは、「あと何割か」を偽って伝えます。
そのステージは数値を出さず、説明ラベルだけを表示します。

単位は表示側が持たず、**数えている側が進捗と一緒に渡します**
（`StageProgress` / `src/types/progress.ts`）。
