...
| `--no-fail-on-incomplete` | 走査が完走しなくても終了コード0にする（既定では3で落とす） |
...
| コード | 意味 | CI側でやること |
| --- | --- | --- |
| `0` | 走査は完走し、閾値以上の検出は無い | そのまま進める |
| `1` | 閾値以上の検出がある | レポートを見てトリアージする |
| `2` | 引数・設定などツール自体のエラー | コマンドを直す |
| `3` | **走査が完走しなかった**（検出0件に意味が無い） | APIキー・ネットワーク・トークン予算を直して再実行する |

`3` を `1` と分けているのは、対応が別だからです。

このスキャナは部分的失敗を許容する設計（LLM呼び出しが失敗しても続行する）なので、
認証エラーやネットワーク断で**全タスクが失敗すると検出0件になります**。
件数だけを見るゲートはこれを緑で通してしまい、「脆弱性が無い」という誤った結論を出します。
そのため検出結果とは独立に「走査が完走したか」を判定し、
完走していなければ検出0件でも `3` を返します。

この判定は `--fail-on` からは独立です。`--fail-on never` は
「検出結果でビルドを落とさない」という指定であって、
「壊れた走査を成功として報告してよい」という指定ではないためです。
どうしても無効化したい場合だけ `--no-fail-on-incomplete`（または
`failOnIncompleteScan: false`）を使ってください。

判定結果は全フォーマットに出ます。JSONなら `result.health`、
SARIFなら `invocations[0].executionSuccessful` と `toolExecutionNotifications`、
CLI・Markdown・HTMLは本文の冒頭に警告が入り、
「検出されませんでした」ではなく「判定できません」という文言に切り替わります。
